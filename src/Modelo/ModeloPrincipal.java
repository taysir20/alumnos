...
public class ModeloPrincipal {
	...
	private InterfaceAccesoDatos modelo; // atributo de interface que usamos
	...
	public void NuevoDato(String tipoDeDato) {
		...
		} else if (tipoDeDato.equalsIgnoreCase("HIBERNATE")) {
		...
		for (int i = 0; i < resultados2.size(); i++) {
			...
			if (resultados2.get(i).getCurso().equals(comboBoxCursos)) {
				alumno = new Alumnos(txtNombre, txtApellido, txtDNI, comboBoxNacionalidad, txtTelefono,
						resultados2.get(i), resultados2.get(i).getId());
			}
		}
	...
	public void actualizarTablaAlumnos(ArrayList<Alumnos> arrayListAlumnos) {
		this.vistaAlumnos.actualizarTablaAlumnos(arrayListAlumnos); // volvemos
																	// a llamar
																	// a recoger
																	// Datos de
																	// la bbdd y
	...
	public void intercambioDatos() {
		...
		this.vistaAlumnos.generarRespuesta(
				"El volcado de datos se producirá desde la Base de Datos a la que estás conectado actualmente. Seleccione a continuación el destino del volcado de datos.");
		...
		} else if (tipoDatos.equals("HIBERNATE")) {
			receptor = new ModeloHibernate();
		}
		receptor.AccesoBBDD();
	...
	public void volcarDatos(ArrayList<Alumnos> emisor) {
		...
		if (String.valueOf(receptor.getClass()).equals("class Modelo.ModeloHibernate")) {
			System.out.println("entra 1");
			Cursos cursoAux;
			for (int i = 0; i < emisor.size(); i++) {
				cursoAux = emisor.get(i).getCurso();
				cursoAux.setId(emisor.get(i).getId_Curso());
				alumno = new Alumnos(emisor.get(i).getCod(), emisor.get(i).getNombre(), emisor.get(i).getApellido(),
						emisor.get(i).getDni(), emisor.get(i).getNacionalidad(), emisor.get(i).getTelefono(), cursoAux,
						emisor.get(i).getId_Curso());
			...
			for (int i = 0; i < emisor.size(); i++) {
				alumno = new Alumnos(emisor.get(i).getCod(), emisor.get(i).getNombre(), emisor.get(i).getApellido(),
						emisor.get(i).getDni(), emisor.get(i).getNacionalidad(), emisor.get(i).getTelefono(),
						emisor.get(i).getCurso(), emisor.get(i).getCurso().getId());
				receptor.addUnAlumno(alumno);
	...
	public void actualizarDatosAlumnos(String cod, String txtNombre, String txtApellido, String txtDNI,
			String comboBoxNacionalidad, int txtTelefono, String comboBoxCurso) {
		System.out.println("el combobox que llega es: " + comboBoxCurso);
		recogerDatosBBDDCursos();
		for (int i = 0; i < resultados2.size(); i++) {
			if (resultados2.get(i).getCurso().equals(comboBoxCurso)) {
				System.out.println("acaso entra??");
				alumno = new Alumnos(Integer.parseInt(cod), txtNombre, txtApellido, txtDNI, comboBoxNacionalidad,
						txtTelefono, resultados2.get(i), resultados2.get(i).getId());
			}
		}
		if (modelo.actualizarJugadores(alumno) == 1) {
	...
	private void volcarDatosCursos(ArrayList<Cursos> emisor) {
		for (int i = 0; i < emisor.size(); i++) {
			...
			curso = new Cursos(emisor.get(i).getCurso(), emisor.get(i).getFechaInicio(), emisor.get(i).getFechaFin(),
					emisor.get(i).getTitulacion());
	...
	public void borrarCursos(String codCurso) {
		...
		this.recogerDatosBBDDCursos();
		this.vistaAlumnos
				.generarRespuesta("Al borrar un curso borrarás todos los alumnos relacionados con él. ¿Estás seguro?");
	...
	// EXTRA. ESTA PARTE ES UN AÑADIDO PARA PODER ESCOGER TU EL FICHERO QUE
	// QUIEROS SUBIR A UNA BBDD.
	// LA SEPARO DEL RESTO DEL CÓDIGO POR NO SÉ EXACTAMENTE COMO PODER
	// IMPLEMENTARLA MEJOR
	/*
	 * public void recogerDatosCualquierFichero(String ruta) {
	 * System.out.println("entra al modelo"); InterfaceAccesoDatos auxiliar =
	 * new ModeloSQL(); auxiliar.AccesoBBDD(); FileReader fr; BufferedReader br;
	 * ArrayList<Alumnos> arrAlumnos = new ArrayList<Alumnos>(); try { File
	 * miFichero = new File(ruta); String[] lineaAlumno = null; if
	 * (miFichero.exists()) { fr=new FileReader(miFichero); br= new
	 * BufferedReader(fr); String miAlumno="";
	 * while((miAlumno=br.readLine())!=null){ lineaAlumno=miAlumno.split("-");
	 * alumno =new
	 * Alumnos(lineaAlumno[2],lineaAlumno[3],lineaAlumno[1],lineaAlumno[4],
	 * Integer.parseInt(lineaAlumno[5])); System.out.println("mi alumno está: "
	 * + alumno); arrAlumnos.add(alumno); System.out.println("añade alumno"); }
	 * System.out.println("sergioooooooo");
	 * 
	 * System.out.println("EL size es :" + arrAlumnos); for (int i = 0; i <
	 * arrAlumnos.size(); i++) { System.out.println("add alumno");
	 * auxiliar.addUnAlumno(arrAlumnos.get(i)); }
	 * 
	 * } else System.err.println("Fichero no encontrado"); } catch (IOException
